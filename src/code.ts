                ...
                if(newchild.type === 'FRAME' && newchild.name === 'voter') {
                  newchild.children.forEach(element => {
                    if(element.name !== msg.name) {
                      // if child name is not equal to chat name, then run function to create
                      // will add arrays for pos + neg and then count length
                      // vote view is created at intial chat creation
                    }
                  });
                }
              })
